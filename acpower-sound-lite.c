...
// state (whether the AC was plugged or unplugged) and play a sound depending of state
//
...
// resource free, and to keep this program in a single file, a shell program is used for playing sound
//
// HOWEVER this is the lite version, so it gets ran ONCE, no loops are in this source code, and it gives the current state of AC instead of checking for state changes
//
...
// This is the file that stores the status of the AC, the "online" provives a 0 or a 1 if it's powered
...
// ##### PROGRAM DEFINES #####
// Main program runs in a while loop, it's highly inneficient to keep reading a file that barely ever changes, save some CPU usage by waiting a little between each loop
#define SLEEP_TIME 1
...
static inline int read_status_file(char *status_file, char out[AC_STATUS_SIZE]){
	...
	if ((read = getline(&line, &len, file)) != -1){
		...
		perror("Unable to read line, file is in use?");
	}
...
// Uses the 2 above functions to get the integer value of the AC status - returns are for erros
...
int main(int argc, char **argv){
	...
	// If state is unplugged - no if needed
	if(fFlags & fVerbose)
